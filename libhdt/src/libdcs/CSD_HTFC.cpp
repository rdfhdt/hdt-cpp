...
CSD_HTFC::CSD_HTFC(hdt::IteratorUCharString *it, uint32_t blocksize,
  ...
  unsigned char *textfc =
      (unsigned char *)malloc(reservedSize * sizeof(unsigned char));
  uint64_t bytesfc = 0;
  ...
  while (it->hasNext()) {
    ...
    if ((bytesfc + currentLength + 1) > reservedSize) {
      ...
      textfc = (unsigned char *)realloc(textfc,
    ...
    if ((numstrings % blocksize) == 0) {
      ...
      // delta << " Difference: " << currentStr + delta << endl << endl;
  ...
  for (uint64_t i = 0; i < tsize; i++)
    text[i] = 0; // Fixme: Replace for calloc
  ...
  while (i < bytesfc) {
    ...
    if ((bytes + maxlength + 1) >= tsize) {
      ...
      for (uint64_t j = bytes + 1; j < tsize; j++)
        text[j] = 0;
    }
    ...
    if (i == xblocks[cblocks]) {
      ...
      first[0] = 0;
      uint fb = 0,
           fo = 0; // Variables managing bytes and offsets in the string 'first'

      while (true) {
        encodeHT(HTcode[(int)textfc[i]].code, HTcode[(int)textfc[i]].cbits,
                 first, &fb, &fo);
        if (textfc[i] == '\0')
          break;
        i++;
      ...
      // **** strncpy((char*)(text+bytes), (char*)first, fb); POR QUE ESTO NO
      // FUNCIONA SIEMPRE BIEN? :(
      for (uint64_t i = 0; i < fb; i++)
        text[bytes + i] = first[i];
      bytes += fb;
  ...
  for (uint i = 0; i < tree->getLength(); i++) {
    if (tree->getBit(i) == 0) {
      ...
      if (traversing.size() > 0) {
        ...
        if (HTtree[parent].symbol == 0)
          HTtree[parent].children[0] = node;
        else
          HTtree[parent].children[1] = node;
...
void CSD_HTFC::dumpBlock(uint block) {
  ...
  while ((idInBlock < blocksize) && (pos < bytes)) {
    // cout << "POS: " << pos << "/" << bytes << " Next block: "<<
    // blocks->getField(block+1)<<endl;
...
void CSD_HTFC::freeString(const unsigned char *str) { delete[] str; }
...
CSD *CSD_HTFC::load(istream &fp) {
  ...
  while (counter < dicc->bytes && fp.good()) {
    fp.read(ptr, dicc->bytes - counter > blocksize ? blocksize
                                                   : dicc->bytes - counter);
    ptr += fp.gcount();
    counter += fp.gcount();
  }
  ...
  for (uint i = 0; i < dicc->tree->getLength(); i++) {
    if (dicc->tree->getBit(i) == 0) {
      ...
      if (traversing.size() > 0) {
        ...
        if (dicc->HTtree[parent].symbol == 0)
          dicc->HTtree[parent].children[0] = node;
        else
          dicc->HTtree[parent].children[1] = node;
...
bool CSD_HTFC::locateBlock(const unsigned char *s, uint *block) {
  ...
  for (uint i = 0; i < slen; i++)
    encodeHT(HTcode[s[i]].code, HTcode[s[i]].cbits, encoded, &encpos,
             &encoffset);
  if (encoffset > 0)
    encpos++;
  ...
  uint delta,
      cmplen; // FIXME: cmplen is assigned, below, but never actually used?
  int cmp;
...
uint CSD_HTFC::locateInBlock(uint block, const unsigned char *s, uint len) {
  ...
  while (read < blocksize && pos < bytes - 1) {
    ...
    if (delta < clcp_len) {
      ...
      plcp_len = clcp_len;
      clcp_len += longest_common_prefix(tmp + clcp_len, s + clcp_len,
                                        tmplen - clcp_len, len - clcp_len);
...
void CSD_HTFC::decompressDelta(unsigned char *seq, uint *pos, uint *offset,
                               unsigned char *deltaseq) {
  uint i = 0;
...
uint CSD_HTFC::decompressFirstWord(unsigned char *seq, uint *pos,
                                   unsigned char *word) {
  uint ptr = 0, offset = 0;
...
uint CSD_HTFC::decompressWord(unsigned char *seq, uint *pos, uint *offset,
...
void CSD_HTFC::encodeHT(uint code, uint len, unsigned char *seq, uint *pos,
                        uint *offset) {
...
uint CSD_HTFC::longest_common_prefix(const unsigned char *str1,
                                     const unsigned char *str2, uint lstr1,
                                     uint lstr2) {
  uint delta = 0;
  uint length = lstr1;
  if (length > lstr2)
...
void CSD_HTFC::fillSuggestions(const char *base, vector<std::string> &out,
                               int maxResults) {
