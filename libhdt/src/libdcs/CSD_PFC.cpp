...
CSD_PFC::CSD_PFC(hdt::IteratorUCharString *it, uint32_t blocksize,
  ...
  while (it->hasNext()) {
    ...
    if ((numstrings % blocksize) == 0) {
      ...
      size_t delta = longest_common_prefix((unsigned char *)previousStr.c_str(),
                                           currentStr, previousStr.length(),
                                           currentLength);
    ...
    NOTIFYCOND(listener, "Converting dictionary to PFC", numstrings,
...
void CSD_PFC::freeString(const unsigned char *str) { delete[] str; }
...
CSD *CSD_PFC::load(istream &fp) {
  ...
  if (crch.getValue() != filecrc) {
    throw std::runtime_error(
        "Checksum error while reading Plain Front Coding Header.");
  }
  ...
  if (dicc->bytes && dicc->numstrings) {
    ...
    while (counter < dicc->bytes && fp.good()) {
      crcd.readData(fp, ptr,
                    dicc->bytes - counter > blocksize ? blocksize
                                                      : dicc->bytes - counter);
    ...
    if (counter != dicc->bytes) {
      throw std::runtime_error(
          "Could not read all the data section of the Plain Front Coding.");
    }
  } else {
  ...
  if (filecrcd != crcd.getValue()) {
    throw std::runtime_error(
        "Checksum error in the data section of the Plain Front Coding.");
  }
...
size_t CSD_PFC::load(unsigned char *ptr, unsigned char *ptrMax) {
  ...
  if (ptr[count++] != PFC)
    throw std::runtime_error(
        "Trying to read a CSD_PFC but type does not match");
...
size_t CSD_PFC::locateInBlock(size_t block, const unsigned char *str,
                              size_t len) {
  if (block >= nblocks) {
    return 0;
  }
  ...
  while ((idInBlock < blocksize) && (pos < bytes)) {
    ...
    if (delta >= commonPrefix) {
      // Compare tmpString with the searched one, only after commonPrefix
      // characters. (We already knew that commonPrefix was common anyway).
      commonPrefix += longest_common_prefix(
          (unsigned char *)tmpStr.c_str() + commonPrefix, str + commonPrefix,
          tmpStr.length() - commonPrefix, len - commonPrefix);
...
size_t CSD_PFC::longest_common_prefix(const unsigned char *str1,
                                      const unsigned char *str2, size_t lstr1,
                                      size_t lstr2) {
  size_t delta = 0;
...
hdt::IteratorUCharString *CSD_PFC::listAll() { return new PFCIterator(this); }
...
void CSD_PFC::fillSuggestions(const char *base, vector<std::string> &out,
                              int maxResults) {
  size_t block = 0;
  ...
  while (block < nblocks && !terminate) {
    ...
    // Scanning the block until a decission about the existence of 's' can be
    // made.
