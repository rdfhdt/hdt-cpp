...
static const size_t DELTA =
    5; // Maxixum possible length for a VByte encoding delta.
...
public:
  ...
  CSD_HTFC(hdt::IteratorUCharString *it, uint32_t blocksize,
  ...
  */
  ...
  /** Obtains the original Tdict from its CSD_HTFC representation. Each string
     is separated by '\n' symbols.
...
protected:
  ...
  BitString
      *tree; //! Hu-Tucker tree shape (ESPECIFICAR COMO CRESTA SE CODIFICA!!!!)
  Node *HTtree; //! Hu-Tucker tree
  uint leafs;   //! Leafs in the Hu-Tucker tree.
  Tcode
      *HTcode; //! Vector assigning Hu-Tucker codes to the symbols in the text.
  bool search;
  ...
  */
  uint longest_common_prefix(const uchar *str1, const uchar *str2, uint lstr1,
                             uint lstr2);
