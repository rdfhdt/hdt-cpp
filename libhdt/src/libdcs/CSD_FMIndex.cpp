...
CSD_FMIndex::CSD_FMIndex(hdt::IteratorUCharString *it, bool sparse_bitsequence,
  ...
  build_ssa((unsigned char *)textFinal, len, sparse_bitsequence, bparam,
            use_sample, bwt_sample);
  if (use_sample) {
...
void CSD_FMIndex::build_ssa(unsigned char *text, size_t len,
                            bool sparse_bitsequence, int bparam,
                            bool use_sample, size_t bwt_sample) {
...
size_t CSD_FMIndex::locate_substring(unsigned char *s, size_t len,
                                     uint32_t **occs) {
  uint32_t dummy;
...
size_t CSD_FMIndex::locate_substring(unsigned char *s, size_t len,
                                     ...
                                     bool deduplicate, uint32_t **occs,
                                     uint32_t *num_occ) {
  if (!use_sampling) {
    *occs = NULL;
    return 0;
...
void CSD_FMIndex::freeString(const unsigned char *str) { delete[] str; }
