...
template <class T> BNode<T>::BNode() {
  w = -1;
  obj = 0;
  pos = -1;
  type = 0;
...
template <class T> BNode<T>::BNode(int v, int p, int t, T *o) {
  w = v;
  obj = o;
  pos = p;
  type = t;
...
template <class T> void BNode<T>::setEndpoints(int start, int end) {
  endpoint[0] = start;
  endpoint[1] = end;
}

template <class T> void BNode<T>::print() {
...
template <class T>
void BNode<T>::code(int level, Tcode **codes, BitString **tree, bool *bits,
                    int *tpos) {
  (*tpos)++;
...
template <class T> void BNode<T>::recPrint(int level) {
