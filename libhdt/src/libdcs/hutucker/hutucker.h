...
/** Auxiliar class to build a OABT using a variant of Hu-Tucker algorithm showed
 * in [1].
 *
...
template <class T> class HuTucker {
public:
...
template <class T> HuTucker<T>::HuTucker(T *array, int szArray) {
#ifdef PRINT
...
template <class T> HuTucker<T>::~HuTucker() {
...
template <class T> BNode<T> *HuTucker<T>::merge(BNode<T> *l, BNode<T> *r) {
...
template <class T> int HuTucker<T>::getPosMin() {
  int min = start;
...
template <class T> void HuTucker<T>::remove(int &pos) {
  seq[pos] = 0;
...
template <class T> int HuTucker<T>::findPosCompatible(int &pmin) {
  int pcom, minleft, minright;
  minleft = minright = pmin;
  // find towards left end of pmin's huffman seq
  if (pmin != start) {
    minleft = pmin - 1;
    for (int left = pmin - 2; left >= start && seq[left + 1]->type != 0;
         left--) {
      if (seq[minleft]->w >= seq[left]->w)
        minleft = left;
    }
  }
  ...
  if (pmin != end) {
    minright = pmin + 1;
    for (int right = pmin + 2; right <= end && seq[right - 1]->type != 0;
         right++) {
      if (seq[minright]->w > seq[right]->w)
        minright = right;
    }
  }
  ...
  if (minleft == pmin)
    ...
    if (minright == pmin)
      ...
      pcom = (seq[minleft]->w <= seq[minright]->w) ? minleft : minright;
  }
  return pcom;
}

template <class T> void HuTucker<T>::combination() {
...
template <class T> void HuTucker<T>::levelAssignment() { recLevelAssign(root); }

template <class T> void HuTucker<T>::recLevelAssign(BNode<T> *curr) {
  static int lvl = 0;
...
template <class T> void HuTucker<T>::recombination() {
...
template <class T> void HuTucker<T>::print() {
  int i;
  cout << "internal nodes: " << weight << ", objects: " << len
       << ", starting at pos: " << start << ", ending at pos:" << end << "\n";
  for (i = 0; i < len; i++)
    if (seq[i]) {
      cout << "(" << i << "): ";
      seq[i]->print();
...
template <class T> uint HuTucker<T>::getCodes(Tcode **codes, BitString **tree) {
...
template <class T> void HuTucker<T>::printLevels() {
  int i;
  cout << "Levels: ";
  for (i = 0; i < len; i++)
    cout << levels[i] << " ";
