...
protected:
  ...
  hdt::LogSequence2
      *blocks;      //! Start positions of each block in the encoded sequence.
...
public:
  PFCIterator(CSD_PFC *pfc) : pfc(pfc), count(1) { max = pfc->getLength(); }

  virtual ~PFCIterator() {}

  bool hasNext() { return count <= max; }

  unsigned char *next() { return pfc->extract(count++); }

  size_t getNumberOfElements() { return max; }

  virtual void freeStr(unsigned char *ptr) { pfc->freeString(ptr); }
};
...
public:
  PFCSuggestionIterator(CSD_PFC *pfc, const char *prefix)
    ...
    pfc->locateBlock(
        ...
        &block); // locate block where the substring might be located
  ...
  void locateInternal() {
    ...
    while (((idInBlock < pfc->blocksize) && (pos < pfc->bytes) && !terminate) &&
           !hasNext()) {
      // Decode the prefix
      if (sizeof(delta) == 8)
        pos += VByte::decode(pfc->text + pos, pfc->text + pfc->bytes,
                             (uint64_t *)&delta);
      else
        pos += VByte::decode(pfc->text + pos, pfc->text + pfc->bytes,
                             (uint32_t *)&delta);
