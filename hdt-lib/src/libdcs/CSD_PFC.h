...
namespace csd
{
static const size_t FACTOR = 20; // Percentual fraction (of the original dict
...
{		
  public:		
    ...
    */
    CSD_PFC(char *filename, size_t blocksize);
	...
	@blocksize: number of strings stored in each dictionary block.
    */
    CSD_PFC(uchar *dict, uint tlength, size_t blocksize);

    CSD_PFC(IteratorUCharString *it, size_t blocksize);
  ...
  protected:
    size_t bytes;	//! Size of the Front-Coding encoded sequence (in bytes).
