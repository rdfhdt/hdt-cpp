...
void setPredicate() {
	...
	if (currpredicate > 0) {
		...
		snprintf(currPredicateStr, BUF_SIZE,
				"Predicate (%d/%d): %s (%d Triples)", currpredicate,
				dictionary->getNpredicates(), pred.c_str(), count);
	} else {
		snprintf(currPredicateStr, BUF_SIZE, "Predicate (ALL/%d)",
...
bool loadHDT(char *hdt) {
	...
	snprintf(dataset_str, BUF_SIZE, "%s (%u Triples)", datasets[currentFile],
			triples->size());
...
void keyboardSpecialDown(int k, int x, int y) {
	...
	case GLUT_KEY_PAGE_UP:
		currpredicate = currpredicate == 0 ? dictionary->getNpredicates() - 1
				: currpredicate - 1;
		setPredicate();
		break;
	case GLUT_KEY_PAGE_DOWN:
		currpredicate = currpredicate >= dictionary->getNpredicates() ? 0
				: currpredicate + 1;
		setPredicate();
		break;
	case GLUT_KEY_HOME:
...
/** Mouse Click
 * @param btn Description of the param.
 * @param state Description of the param.
 * @param x Description of the param.
 * @param y Description of the param.
 * @return void
 */
...
/** Dist
 * @param x1 Description of the param.
 * @param x2 Description of the param.
 * @param y1 Description of the param.
 * @param y2 Description of the param.
 * @return The expected result
 */
 ...
 * @return The expected result
 */
TripleID *findTripleBinSearch(vector<TripleID> &graph, unsigned int x,
	...
	do {
		...
		jumps++;
	} while ((min < max) && ((graph[mid].x != x) || (graph[mid].y != y)
			|| (graph[mid].z != z)));
 ...
 * @return The expected result
 */
TripleID *findTripleSeq(vector<TripleID> &graph, unsigned int x,
		unsigned int y, unsigned int z) {
...
void mousePassiveMotion(int x, int y) {
	...
	glScalef(1.0f / (float) nobjects, 1.0f / (float) nsubjects, 1.0f
			/ (float) npredicates);
	...
	//printf("Read z: %f\n", wz);
	gluUnProject(wx, wy, currpredicate, modelview, projection, viewport, &ox,
			&oy, &oz);
	//printf("Orig: %f %f %f\n", ox, oy, oz);
	...
	foundTriple = findTripleSeq(triples->getGraph(), currpredicate, subject,
			object);
	predicate = foundTriple->x;
	subject = foundTriple->y;
	object = foundTriple->z;

	if ((subject > 0 && subject < nsubjects) && (object > 0 && object
			<= nobjects)) {
		...
		snprintf(mouse_str_suj, BUF_SIZE, "S: %6u / %s", subject,
				subjStr.c_str());
		snprintf(mouse_str_pred, BUF_SIZE, "P: %6u / %s", predicate,
				predStr.c_str());
...
/** Texto
 * @param cadena Description of the param.
 * @param x Description of the param.
 * @param y Description of the param.
 * @param z Description of the param.
 * @return void
...
void draw() {
	...
	for (int i = 0; (i < graph.size()) && (i < currFrame); i += increment) {
		...
		if (currpredicate == 0 || currpredicate == graph[i].x) {
			...
			glVertex3f(((float) graph[i].z), ((float) graph[i].y),
					((float) graph[i].x));
		}
	}
	glEnd();
	...
	if (foundTriple != NULL) {
		...
		glVertex3f(foundTriple->z - sizex, foundTriple->y - sizey,
				foundTriple->x);
		glVertex3f(foundTriple->z + sizex, foundTriple->y + sizey,
				foundTriple->x);
		glVertex3f(foundTriple->z - sizex, foundTriple->y + sizey,
...
/** Timer
 * @param valor Description of the param.
 * @return void
 */
...
/** Init GL
 * @param width Description of the param.
 * @param height Description of the param.
 * @return void
 */
