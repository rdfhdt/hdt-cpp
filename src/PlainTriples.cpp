 ...
 */
PlainTriples::PlainTriples(Dictionary *dictionary, unsigned int ntriples,
 ...
 */
bool PlainTriples::loadGraphMemory() {
	...
	if (read_z == ntriples) {
		...
		cout << endl
				<< "   <ERROR> The number of read triples does not match up with the expected one"
				<< endl;
		return false;
	}
}
 ...
 * @return The expected result
 */
bool PlainTriples::transformToN3() {
	unsigned int x, y, z;
	size_t read_x = 0, read_y = 0, read_z = 0;
	...
	if (read_z == ntriples) {
		...
		cout << endl
				<< "   <ERROR> The number of transformed triples does not match up with the expected one"
				<< endl;
		return false;
	}
}
 ...
 * @return The expected result
 */
bool PlainTriples::serialize(char *output, char *format) {
 ...
 * @return The expected result
 */
unsigned int PlainTriples::write(vector<TripleID> &graph, string path) {
	string output = path;
 ...
 * @return The expected result
 */
unsigned int PlainTriples::write(string path) {
