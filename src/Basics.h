...
enum ParsingType {
	SPO = 1, SOP, PSO, POS, OSP, OPS
};

static const char *parsingTypeStr[] = { "", "SPO", "SOP", "PSO", "POS", "OSP",
		"OPS" };
...
static getTime(DataTime *t) {
	...
	t->user = (double) buffer.ru_utime.tv_sec + 1.0e-6
			* buffer.ru_utime.tv_usec;
	t->total = (double) tp.tv_sec + 1.0e-6 * tp.tv_usec;
}

/** Print the credits
 * @return the credits printed into cout
 */
 ...
 * @return the help printed into cout
 */
 ...
 * @return the help of the console printed into cout
 */
void static printHelpConsole() {
	cout
			<< "        parse <config_file>            Parse data set within the given <config_file>\n";
	cout
			<< "        load <file>                    Load HDT structure into memory and access triples console\n";
	cout << "        help                           Show this help\n";
	cout << "        exit                           Exit console mode\n";
}

/** Splits a string with an specific separator
 * @param str Description of the param.
 * @param separator Description of the param.
 * @param results Description of the param.
 * @return void
 */
