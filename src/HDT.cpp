	...
	while (true){
		...
		for (i=0;i<subcommand.size();i++){
			...
			if (subcommand[i]=="parse"){ 
				...
				cout<<"\n"<<subcommand[i]<<": command not found. Type help for commands";
				break;
			}
		}
		subcommand.clear();
...
main(int argc, char* argv[])
{
	...
	{			
		...
		else
		{
			...
			{	
				...
				if (argc>=4){
					...
					if (transform==true){
						loader->serialize((char *)output.c_str(),(char *)format.c_str());
					}
					else{
						cout << "   <ERROR> Wrong parameters.\nFor help, type: hdt -h\n\nEntering Console mode..." << endl;
						loader->console();
		...
		else if (strcmp(argv[1],"-c")==0 || strcmp(argv[1],"--console")==0){
			console();
		}
		else
		{
